...
- `deploy/kong/kong.yml` originally declared an identical `/health` path under all three services' routes (`user-public`, `product-public`, `rental-public`). Kong's router resolves path collisions by its own priority rules (regex specificity / config order), not by which container is actually reachable, and it was resolving all three to `user-service` — breaking the health check for the other two. This was found and fixed during this plan's execution: the `product-public`/`rental-public` route stanzas were removed from `kong.yml`, leaving only `user-public`'s `/health`. The remaining risk is now forward-looking: whoever adds `product-service`/`rental-service` back into Kong later should give each a **distinct** health path (e.g. `/product/health`, `/rental/health`) rather than reusing the shared `/health` string, to avoid reintroducing the same collision.
