...
| วิษณุพงศ์ บัวเขียว | 67114540509 | ระบบจัดการการเช่า | `rental-service/` | ⏳ ยังไม่เริ่ม (มีแค่ README) |
...
- **user-service เท่านั้นที่ถือ `JWT_SECRET`** และเป็นคนออก token — product/rental (เมื่อ
  มีโค้ดแล้ว) จะแค่ decode payload อ่าน claims เอา ไม่ต้อง verify signature ซ้ำ (ดู
  [CONTRACT.md §5.3](CONTRACT.md))
...
├── rental-service/      # :8083  → rental-db    ⏳ ยังไม่มีโค้ด
├── deploy/
│   └── kong/            # kong.yml, smoke-test.sh, README
├── docs/
...
├── .env.example
└── docker-compose.yml   # ตอนนี้มีแค่ user-service + user-db + kong
```
...
docker compose ps                          # user-service, user-db, kong ต้อง healthy/running
curl http://localhost:8000/health          # ผ่าน Kong -> 200
./deploy/kong/smoke-test.sh                # ทดสอบครบในคำสั่งเดียว -> 6 passed, 0 failed
```
...
## เมื่อ product-service หรือ rental-service เริ่มมีโค้ดจริง

ทำตามลำดับนี้ (เขียนจากมุมมองของเจ้าของ service ที่กำลังจะเพิ่มเข้ามา):

### 1. เขียน service ตาม CONTRACT.md ก่อน

- อ่าน endpoint ที่ต้อง expose: [CONTRACT.md §8.2](CONTRACT.md) (product) หรือ
  [§8.3](CONTRACT.md) (rental) — path, method, สิทธิ์ต่อ role ตามตารางนั้น
...
เพิ่ม service + db ของตัวเอง ตามแพทเทิร์นเดียวกับ `user-service`/`user-db` ที่มีอยู่แล้ว
(ตัวแปร `PRODUCT_APP_PORT`/`PRODUCT_DB_*` หรือ `RENTAL_APP_PORT`/`RENTAL_DB_*` มีอยู่ใน
`.env.example` แล้ว รอแค่ service block):

```yaml
  product-service:                    # หรือ rental-service
...
`rental-service` ตอนนี้โดยตั้งใจ — เพราะถ้าประกาศ path `/health` แบบเดียวกันซ้ำกันทั้ง 3
service Kong จะ route ผิดไปที่ service อื่น (เจอบั๊กนี้มาแล้วจริงตอน implement Kong ตอนแรก)
...
`product-protected`/`rental-protected` (route หลักที่ใช้งานจริง) **มีอยู่แล้วครบ** ใน
...
รัน `./deploy/kong/smoke-test.sh` อีกครั้งด้วย — ต้องยังผ่าน 6/6 เหมือนเดิม (ไม่มีอะไรพัง)
